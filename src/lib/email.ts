// Email service for OISDRIVE contact form using Mailgun
...
// Main function to handle contact form email sending
export async function sendContactFormEmails(data: ContactFormData): Promise<{
  ...
  try {
    ...
    const adminEmailSent = await sendEmail('contact@oisdrive.com', templates.adminEmail)
    
    // Send confirmation email to user
    const userEmailSent = await sendEmail(data.email, templates.userEmail)
